...
// CSS format
var compressCss = require('gulp-css-format-oneline');
...
const regex_cssComments = /\/\*.+?\*\//g; // single line 
const regex_cssAllComments = /\/\*([\s\S]*?)\*\//g; // single and multi line 
const regex_cssDevComments = /\/\*\*.+?\*\//g; // single line dev comments starting /** 
const regex_htmlComments = /<!--(?!<!)[^\[>].*?-->/g; // html comments not including conditional code 
const regex_regex_htmlDevComments = /<!---(?!<!)[^\[>].*?-->/g; // html dev comments starting <!--- not including conditional code 
...
gulp.task('css:comments-strip', function () {
      return gulp
        .src(['src/compiled/index.html'])
        .pipe(replace(regex_cssAllComments, ''))  // Strip CSS comments 
        .pipe(replace(regex_htmlAllDevComments, ''))  // Strip HTML Dev comments
        .pipe(gulp.dest('src/compiled/'));
  });
  

//  CSS FORMAT - compact but readible CSS
// --------------------------------------------------
...
/*  
  ...
  gulp.task('css:format', function() {
  return gulp
   .src('./src/compiled/css/resets.css')
   .pipe(compressCss({
    clearLine: false,
        merge: false
   }))
   .pipe(gulp.dest('src/compiled/compiled/'))
});*/
